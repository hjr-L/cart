                ...
                <div class="item-list-wrap">
                    <div class="cart-item">
                        ...
                        <ul class="cart-item-list">
                            <li v-for="good of goodsList" :key="good.productId">
                                <div class="cart-tab-1">
                                    <div class="cart-item-check">
                                        <a href="javascript:;" class="checkbox-btn item-check-btn "
                                            :class="{'checked' : good.checked}" @click="editCart('',good)">
                                    ...
                                    <div class="cart-item-pic">
                                        <img :src="require('../assets/imgs/'+good.productImage)">
                                    </div>
                                    <div class="cart-item-title">
                                        <div class="item-name">{{good.productName}}</div>
                                    </div>
                                </div>
                                <div class="cart-tab-2">
                                    <div class="item-price">{{good.productPrice}}</div>
                                </div>
                                <div class="cart-tab-3">
                                    <div class="item-quantity">
                                        <div class="select-self select-self-open">
                                            <div class="select-self-area">
                                                <a class="input-sub" @click="editCart('minus',good)">-</a>
                                                <span class="select-ipt">{{good.productNum}}</span>
                                                <a class="input-add" @click="editCart('add',good)">+</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="cart-tab-4">
                                    <div class="item-price-total">{{(good.productNum*good.productPrice) | currency}}
                                    </div>
                                </div>
                                <div class="cart-tab-5">
                                    <div class="cart-item-opration">
                                        <a href="javascript:;" class="item-edit-btn" @click="showModal(good)">
                    ...
                    <div class="cart-foot-inner">
                        ...
                        <div class="cart-foot-r">
                            <div class="item-total">
                                总价: <span class="total-price">{{checkMoney | currency}}</span>
                            </div>
                            <div class="btn-wrap">
                                <a class="btn btn--red  " :class="{'btn--dis':!checkCount}" @click="checkOut">结算</a>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    export default {
        ...
        computed:{
            checkAllFlag(){
                return this.goodsList.every((good)=>{
                    return good.checked;
                })
            },
            checkMoney(){
                ...
                this.goodsList.forEach((good)=>{
                    if(good.checked){
                        money+=good.productNum*good.productPrice;
                    }
                });
                return money;
            },
            //是否有选中商品
            checkCount(){
                return this.goodsList.some((good)=>{
                    return good.checked;
                })
            }
        },
        methods: {
            ...
            //删除数据
            delCart(){
                let delGood = this.delGood;
                this.goodsList.forEach((good,index)=>{
                    if(good.productId==delGood.productId){
